...
func TestAddHeaders(t *testing.T) {
	tests := []struct {
		...
		{"set Info headers on HTTP",
			&http.Request{RemoteAddr: "1.2.3.4:5555", Proto: "HTTP/1.0"},
			...
			http.Header{
				...
				"X-Forwarded-Port":    []string{"80"},
				"X-Real-Ip":           []string{"1.2.3.4"},
				"X-Fabio-Proto":       []string{"HTTP/1.0"},
			},
			"",
		},
		{"set Info headers on TLS",
			&http.Request{RemoteAddr: "1.2.3.4:5555", Proto: "HTTP/1.0", TLS: &tls.ConnectionState{Version: tls.VersionTLS10, CipherSuite: tls.TLS_ECDHE_ECDSA_WITH_CHACHA20_POLY1305 }},
			config.Proxy{InfoHeaderPrefix: "X-Fabio"},
			http.Header{
				"Forwarded":           []string{"for=1.2.3.4; proto=https"},
